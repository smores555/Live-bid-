...
function runBidEngine(data, deltaMap) {
    ...
    Object.keys(liveHeadcount).forEach(key => {
        ...
        vacPool[key] = deltaMap[key] || 0; // Net delta initialization
    });
    ...
    function tryAward(sen, target, prefOrder) {
        ...
        if (rank > pr.bpl) {
            ...
            auditTrail.push({
                txId: txId++, sen: p.sen, name: p.name, from: `737 ${assignments[sen].replace('-', ' ')}`, to: `737 ${target.replace('-', ' ')}`,
        ...
        if (isMoving && vacPool[target] <= 0) {
            p.failedPrefs.push({ order: pr.order, targetKey: target, reason: `Requested position has ${vacPool[target] || 0} vacancy and cannot accept additional pilots.` });
            auditTrail.push({
                txId: txId++, sen: p.sen, name: p.name, from: `737 ${assignments[sen].replace('-', ' ')}`, to: `737 ${target.replace('-', ' ')}`,
                log: { step: 'DENY', prefOrder: pr.order, toKey: target, reason: `Requested position has ${vacPool[target] || 0} vacancy and cannot accept additional pilots.` }
            });
            return false;
        }
    ...
    Object.keys(targetMap).forEach(key => {
        ...
        if (atKey.length > cap) {
            minSensAtCap[key] = cap > 0 ? atKey[cap - 1].sen : (atKey[0] ? atKey[0].sen : 0);
        }
    });
    ...
    while (cascadePhase3 && displacedQueue.length > 0 && p3Loops < 5000) {
        ...
        for (const p of displacedQueue) {
            ...
            if (newSeat !== p.currentKey) {
                ...
                assignments[p.sen] = newSeat; // Keep assignments map in sync
                auditTrail.push({
